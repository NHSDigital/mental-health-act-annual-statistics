# Databricks notebook source
 ...
 ---creates table with every possible Breakdown, Demographic Breakdown and Organisation combination
 (
 Breakdown string,
...
# COMMAND ----------
 ...
 ---inserts all ccg/subicb values for the financial year
...
# COMMAND ----------
 ...
 %sql
 insert into $db_output.mha_ref_master
 ---inserts all stp/icb values for the financial year
 select 
 distinct
 'STP' as Breakdown,
 'All' as Demographic_Breakdown,
...
# COMMAND ----------
 ...
 ---inserts all provider for the financial year from both mhsds and ecds
 insert into $db_output.mha_ref_master
 select 
 ...
 where od.NAME is not null ---valid providers for the financial year only
...
# COMMAND ----------
 ...
 refresh table $db_output.mha_rd_org_daily_latest;
 insert into $db_output.mha_ref_master
 ---inserts all provider for the financial year from mhsds
 select 
...
# COMMAND ----------
 ...
 %sql
 insert into $db_output.mha_ref_master
 ---inserts all stp/icb and demographic values (higher ethnicity, age band, gender, deprivation) combinations for the financial year
...
#json for each measure and sub-measure with all of the associated breakdowns for that measure and sub-measure
mha_measures_metadata = {
  ...
  "Detentions on admission to hospital": {
    "measure_sub": {
      "All" : ["All submissions", "Independent Health Provider Both", "NHS Trust Both"],
      "All detentions under Part 2" : ["All submissions", "Independent Health Provider Total", "NHS Trust Total"],
      "All detentions under Part 3" : ["All submissions", "Independent Health Provider Total", "NHS Trust Total"],
      ...
      "Section 2" : ["All submissions", "Independent Health Provider Total", "NHS Trust Total"],
      "Section 3" : ["All submissions", "Independent Health Provider Total", "NHS Trust Total"],
...
# COMMAND ----------
 ...
 ---final csv "skeleton" which has each Measure, Sub-measure, demographic breakdown, organisation breakdown
 (
 Year string,
 Measure string,
...
def insert_into_csv_master(ref_df, year, measure, sub_measure, breakdown):
  """
  Using the $db_output.mha_ref_master table and mha_measures_metadata values
  create a dataframe which will get the measure/sub-measure/breakdown values
  then insert this dataframe into $db_output.mha_csv_master_prep
  """
  csv_df = (
    ref_df
...
#loop through mha measures metadata to create final $db_output.mha_csv_master_prep with required measure/sub-measure and associated organisation and demographic breakdowns
...
# COMMAND ----------
 ...
 ---combines all NHS Trust/Independent Provider levels into a single OrganisationBreakdown
 select
 Year,
 Measure,
 MeasureSubCategory,
...
# COMMAND ----------
 ...
 ---combines mha_csv_master table created above with mha_unformatted table created in Agg, ecds_agg table created in Agg and mha_final_rates created in Rates all together into one table (unsuppressed)
 SELECT 
 DISTINCT
 Z.Year,
 Z.Measure,
 ...
 CASE
   WHEN COALESCE(A.METRIC_VALUE, R.count) IS NOT NULL AND F.COUNT IS NOT NULL THEN 'Both' ---has a mhsds and ecds value
   WHEN COALESCE(A.METRIC_VALUE, R.count) IS NOT NULL THEN 'MHSDS' ---mhsds value only
   WHEN F.COUNT IS NOT NULL THEN 'ECDS' ---ecds value only
 ...
 Z.OrgID,
 Z.OrgName,
 COALESCE(a.METRIC_VALUE, R.count) AS MHSDS_COUNT, ---Adding in "All" for demographic breakdowns
 F.COUNT AS ACUTE_COUNT,
 CASE
   WHEN COALESCE(A.METRIC_VALUE, R.count) IS NOT NULL AND F.COUNT IS NOT NULL THEN COALESCE(A.METRIC_VALUE, R.count) + F.COUNT 
   WHEN COALESCE(A.METRIC_VALUE, R.count) IS NOT NULL THEN COALESCE(A.METRIC_VALUE, R.count)
   WHEN F.COUNT IS NOT NULL THEN F.COUNT
   WHEN COALESCE(A.METRIC_VALUE, R.count) IS NULL AND F.COUNT IS NULL THEN 0
   ELSE 'ERROR'
   END AS Total_Count,
 ...
 LEFT JOIN $db_output.mha_final_rates C on z.Measure = C.count_of ---Bring in CCG/STP top-level rates
   ...
   and C.demographic_breakdown = "All"
...
# COMMAND ----------
 ...
 ---suppresses sub-national level counts in $db_output.mha_raw
 SELECT 
 Year,
 Measure,
 measureSubcategory,
 ...
 COALESCE(CASE 
           ...
           ELSE ROUND(MHSDS_Count/5.0,0)*5 ---mhsds suppression logic
           END,
           CASE 
 ...
 COALESCE(CASE 
           ...
           ELSE ROUND(Acute_Count/5.0,0)*5 ---ecds suppression logic
           END,
           CASE 
             WHEN OrgID = 'All prov' THEN 0
             ELSE '*'
 ...
 COALESCE(CASE 
           ...
           ELSE CAST(ROUND(Total_Count/5.0,0)*5 as int) ---mhsds and ecds combined suppression logic
           END,
           CASE 
             WHEN OrgID = 'All prov' THEN 0
             ELSE '*'
