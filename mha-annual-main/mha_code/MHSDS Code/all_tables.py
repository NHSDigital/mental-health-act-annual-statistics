# Databricks notebook source
 ...
 UNION
 ---account for providers who have closed/merged in the year or when a provider site was used as the submitter (for 21/22 it was RTV, ATM01, ATM02)
 SELECT DISTINCT
 ORG_CODE
 ,NAME
 ,CASE 				
...
# COMMAND ----------
 ...
 ---get latest CCG/SubICB Reference data as of the end of the financial year
 SELECT 
 DISTINCT 
   ORG_CODE, 
   NAME
...
# COMMAND ----------
 ...
 INNER JOIN (SELECT 
               Person_ID, 
               MAX(RecordNumber) AS RecordNumber ---Use MAX(RecordNumber) to get latest record for Person_ID
  ...
  WHERE 
 (GP.RecordEndDate is null or GP.RecordEndDate >= '$rp_enddate') and GP.RecordStartDate BETWEEN '$rp_startdate' AND '$rp_enddate' ---get GP records in the financial year only
...
# COMMAND ----------
 ...
 ---apply CCG/SubICB of Practice or Residence logic
 SELECT 
 a.Person_ID, 
 CASE 
   WHEN b.OrgIDCCGGPPractice IS not null and e.ORG_CODE is not null then b.OrgIDCCGGPPractice ---If GP Practice is present and corresponding CCG is valid then use OrgIDCCGGPPractice
   WHEN a.OrgIDCCGRes IS NOT NULL and c.ORG_CODE is not null then  a.ORGIDCCGRes ---If OrgID of Residence is present and corresponding CCG is valid then use OrgIDCCGGPPractice
   ELSE 'UNKNOWN' 
   END as IC_Rec_CCG
 FROM $db_source.mhs001mpi AS a
 LEFT JOIN $db_source.mhs002gp AS b
...
# COMMAND ----------
 ...
 ---final ccg reference data  
 SELECT 
 Person_ID,
 CASE 
   WHEN b.ORG_CODE is NULL THEN 'UNKNOWN' ---if CCG/SubICB code is null or not in corporate reference data then bin into UNKNOWN
   ELSE b.ORG_CODE 
   END AS IC_Rec_CCG,
 CASE 
   WHEN NAME IS null THEN 'UNKNOWN' ---if CCG/SubICB name is null or not in corporate reference data then bin into UNKNOWN
   ELSE NAME 
   END AS NAME
...
# COMMAND ----------
 ...
 ---get latest OrgIDProv, Person_ID combination for a Person in the financial year
 SELECT			 
 				B.orgidProv
 				,B.Person_ID
...
# COMMAND ----------
 ...
 SELECT			 
                 ...
                 LEFT JOIN [DATABASE].[DEPRIVATION_REF] r on r.LSOA_CODE_2011 = B.LSOA2011 AND r.IMD_YEAR = '2019'
 				where (b.RecordEndDate is null or b.RecordEndDate >= '$rp_enddate') and b.RecordStartDate BETWEEN '$rp_startdate' AND '$rp_enddate' ---MPI records in the financial year only
...
# COMMAND ----------
 ...
 ---get all MHA records for the financial year
 SELECT			 
 				B.UniqMonthID
 				,B.orgidProv
 				,B.Person_ID
 ...
 FROM			$db_source.MHS401MHActPeriod AS B
 				where (RecordEndDate is null or RecordEndDate >= '$rp_enddate') and RecordStartDate BETWEEN '$rp_startdate' AND '$rp_enddate' ---MHA records in the financial year only
...
# COMMAND ----------
 ...
 ---get latest conditional discharge records for the financial year
 SELECT			 
                  B.MHS403UniqID
 				,B.UniqMonthID
 				,B.orgidProv
 ...
 FROM			$db_source.MHS403ConditionalDischarge
 					AS B
 				where (RecordEndDate is null or RecordEndDate >= '$rp_enddate') and RecordStartDate BETWEEN '$rp_startdate' AND '$rp_enddate' ---conditional discharge records for the financial year only
...
# COMMAND ----------
 ...
 ---get latest community treatment order records for the financial year
 ...
 FROM			$db_source.MHS404CommTreatOrder
 					AS B
 				where (RecordEndDate is null or RecordEndDate >= '$rp_enddate') and RecordStartDate BETWEEN '$rp_startdate' AND '$rp_enddate' ---community treatment order records for the financial year only
...
# COMMAND ----------
 ...
 ---get latest community treatment order recall records for the financial year
 ...
 FROM			$db_source.MHS405CommTreatOrderRecall
 					AS B
 				where (RecordEndDate is null or RecordEndDate >= '$rp_enddate') and RecordStartDate BETWEEN '$rp_startdate' AND '$rp_enddate' ---community treatment order recall records for the financial year only
...
# COMMAND ----------
 ...
 ---get latest hospital spell records for the financial year
 SELECT			 
 				 B.UniqMonthID
 				,B.orgidProv
 				,B.Person_ID
 ...
 FROM			$db_source.MHS501HospProvSpell
 					AS B
 				where (RecordEndDate is null or RecordEndDate >= '$rp_enddate') and RecordStartDate BETWEEN '$rp_startdate' AND '$rp_enddate' ---hospital spell records for the financial year only
...
# COMMAND ----------
 ...
 %sql
 INSERT INTO $db_output.restrictive_lookup VALUES
...
# COMMAND ----------
 ...
 %sql
 INSERT INTO $db_output.excel_lod_mha_lookup VALUES
 ('All', 'All Detentions'),
 ('All', 'Part II'),
...
# COMMAND ----------
 ...
 %sql
 INSERT INTO $db_output.restrictive_lookup_mha_cto VALUES
 ('31','34','Formally detained under Mental Health Act Section 45A','Court and prison disposals'),
 ('31','37','Formally detained under Mental Health Act Section 45A','Court and prison disposals'),
 ('30','15','Formally detained under Mental Health Act Section 47 with section 49 restrictions','Court and prison disposals'),
 ('29','17','Formally detained under Mental Health Act Section 48 with section 49 restrictions','Court and prison disposals'),
...
# COMMAND ----------
 ...
 %sql
 INSERT INTO $db_output.excel_lod_mha_cto_lookup VALUES
 ('All', 'All Detentions'),
 ('All', 'Part II'),
 ('Formally detained under Mental Health Act Section 2','Part II'),
 ('Formally detained under Mental Health Act Section 3','Part II'),
...
# COMMAND ----------
 ...
 %sql
 DROP TABLE IF EXISTS $db_output.mha_cto_los;
 CREATE TABLE IF NOT EXISTS $db_output.mha_cto_los
 (
 Geography string,
 OrgCode string,
 OrgName string,
