...
# COMMAND ----------
 ...
 ---get short term orders count at lower ethnicity, gender and lower age groups where all demographic values are known and valid
 select 
 distinct
 CASE 
...
# COMMAND ----------
 ...
 ---combine short term orders and population count and calculate crude rate at lower ethnicity, gender and lower age groups where all demographic values are known and valid
 select 
...
# COMMAND ----------
 ...
 ---join short term orders count, population, standardised rate and confidnce interval at lower ethnicity
 select
 a.Ethnicity,
...
# COMMAND ----------
 ...
 %sql
 create or replace temporary view a_rates_final as
...
# COMMAND ----------
 ...
 ---get short term orders count at higher ethnicity, gender and lower age groups where all demographic values are known and valid
 select 
 distinct
 CASE
...
# COMMAND ----------
 ...
 ---join shor term orders count, population, standardised rate and confidence interval at higher ethnicity
 select
 a.Ethnicity,
 sum(c.count) as count,
 sum(c.population) as population,
...
# COMMAND ----------
 ...
 %sql
 CREATE OR REPLACE TEMPORARY VIEW b_rates_final AS
 ...
 count,
 population,
 round((count / population) * 100000, 1) as crude_rate_per_100000,
 round(standardised_rate_per_100000, 1) as standardised_rate_per_100000,
 round(CI, 1) as CI
 from b_rates
 order by Ethnicity
...
# COMMAND ----------
 ...
 %sql
 CREATE OR REPLACE TEMPORARY VIEW c_rates_final AS
 ...
 sum(count) as count,
 sum(population) as population,
 round((sum(count) / sum(population)) * 100000, 1) as crude_rate_per_100000,
...
# COMMAND ----------
 ...
 %sql
 DROP TABLE IF EXISTS $db_output.eth_stos_rates;
 CREATE TABLE IF NOT EXISTS $db_output.eth_stos_rates AS
 ---combine all short term orders figures for lower ethnicity, higher ethnicity and ethnicity total
