# Databricks notebook source
 ...
 ---gets latest mpi and ccg date for a person in March of the financial year
     SELECT MPI.*
            ,CCG.IC_Rec_CCG
           ,CCG.NAME
  ...
  LEFT JOIN global_temp.CCG CCG
      ...
      WHERE UniqMonthID = '$month_id'
            AND MPI.PatMRecInRP = true ---latest submitted person_id record in a single month

# COMMAND ----------

 %sql
 CREATE OR REPLACE GLOBAL TEMPORARY VIEW MHS101Referral_open_end_rp AS
 ---gets open referrals for March of the financial year
     SELECT ref.*
...
# COMMAND ----------
 ...
 ---gets open Section 4, Section 5(2), Section 5(4), Section 135, Section 136 detentions for March of the financial year
     SELECT	*
       FROM	$db_source.MHS401MHActPeriod AS STO
      WHERE  STO.UniqMonthID = '$month_id' 
             AND STO.legalstatuscode IN ('04', '05', '06', '19', '20') ---Section 4, Section 5(2), Section 5(4), Section 135, Section 136 formal detentions only
...
# COMMAND ----------
 ...
 ---joins open referrals, open detentions and mpi data for March of financial year
...
# COMMAND ----------
 ...
 ---gets people subject to a community treatment order of conditional discharge for March of the financial year
     SELECT	DISTINCT REF.Person_ID
 			,REF.RecordNumber
             ,AMHServiceRefEndRP
             ,CYPServiceRefEndRP
 ...
 INNER JOIN  global_temp.MHS001MPI_latest_month_data MPI
             ON REF.Person_ID = MPI.Person_ID
      WHERE	(MHA.EndDateMHActLegalStatusClass IS NULL OR MHA.EndDateMHActLegalStatusClass > '$rp_enddate') ---open detentions for March of financial year
 			AND (CTO.Person_ID IS NOT NULL OR CTOR.Person_ID IS NOT NULL OR CD.UniqMHActEpisodeID IS NOT NULL) ---community treatment order, community treatment order recall or conditional discharge records only
...
# COMMAND ----------
 ...
 ---gets open hospital spells for March of the financial year
     SELECT HSP.*
...
# COMMAND ----------
 ...
 ---gets open ward stays for March of the financial year
     SELECT WRD.*
...
# COMMAND ----------
 ...
 ---gets people subject to detention for March of the financial year
     SELECT	DISTINCT REF.Person_ID
 			,REF.RecordNumber
             ,AMHServiceWSEndRP
             ,CYPServiceWSEndRP
 ...
 INNER JOIN   global_temp.MHS001MPI_latest_month_data MPI
   ...
   LEFT JOIN global_temp.MHS502WardStay_Open_End_RP AS WRD
      ...
      WHERE	(MHA.EndDateMHActLegalStatusClass IS NULL OR MHA.EndDateMHActLegalStatusClass > '$rp_enddate')
 			AND (CTO.Person_ID IS NULL AND CTOR.Person_ID IS NULL AND CD.UniqMHActEpisodeID IS NULL) --- filter for no records for community treatment order, community treatment order recall or conditional discharge records
 			AND STO.RecordNumber IS NULL
             AND MHA.Person_ID IS NOT NULL ---MHA records only

# COMMAND ----------

 %sql
 CREATE OR REPLACE GLOBAL TEMPORARY VIEW MHS401MHActPeriod_GRD_open_end_rp AS 
 ---gets open guardianship (Section 7 and Section 37) detentions for March in the financial year
     SELECT	*
...
# COMMAND ----------
 ...
 ---gets people subject to mental health act for March of the financial year
 SELECT                 ref.Person_ID, 
                        ref.RecordNumber
                   FROM global_temp.MHS101Referral_open_end_rp as REF
             INNER JOIN global_temp.MHS401MHActPeriod_GRD_open_end_rp MHA
                        ON REF.Person_ID = MHA.Person_ID
                  UNION SELECT Person_ID, RecordNumber	FROM $db_output.mha_mhs09_prep ---people subject to detention
                  UNION SELECT Person_ID, RecordNumber	FROM global_temp.MHS10_INTERMEDIATE ---people subject to community treatment order, community treatment order recall or conditional discharge
                  UNION SELECT Person_ID, RecordNumber	FROM global_temp.MHS11_INTERMEDIATE ---people subject to short term holding powers including guardianship  
...
# COMMAND ----------
 ...
 ---gets people subject to community treatment order, community treatment order recall or conditional discharge only in March of the financial year
 ...
 FROM global_temp.MHS10_INTERMEDIATE AS MHS10 
   ...
   LEFT JOIN $db_output.mha_mhs09_prep AS MHS09 
       ...
       WHERE MHS11.Person_ID IS NULL ---no guardianship records
 			AND MHS09.Person_ID IS NULL ---no people subject to detention records

# COMMAND ----------

 %sql
 CREATE OR REPLACE GLOBAL TEMPORARY VIEW MHS11Prov_INTERMEDIATE AS
 ---joins open referrals, open detentions and mpi data for March of financial year by provider
     SELECT	DISTINCT REF.Person_ID
 			,REF.RecordNumber
 			,REF.OrgIDProv
             ,AMHServiceRefEndRP
...
# COMMAND ----------
 ...
 ---gets people subject to a community treatment order of conditional discharge for March of the financial year by provider
     SELECT	DISTINCT REF.Person_ID
 			,REF.RecordNumber
 			,REF.OrgIDProv
...
# COMMAND ----------
 ...
 ---gets people subject to detention for March of the financial year by provider
     SELECT	DISTINCT REF.Person_ID
 			,REF.RecordNumber
 			,REF.OrgIDProv
             ,CASE WHEN REF.OrgIDProv LIKE 'R%' OR REF.OrgIDProv LIKE 'T%' THEN 'NHS Trust' ELSE 'Independent Health Provider' END AS ProvType
...
# COMMAND ----------
 ...
 ---gets people subject to mental health act for March of the financial year by provider
 SELECT                 REF.Person_ID, 
                        REF.RecordNumber,
                        REF.OrgIDProv,
                        CASE WHEN REF.OrgIDProv LIKE 'R%' OR REF.OrgIDProv LIKE 'T%' THEN 'NHS Trust' ELSE 'Independent Health Provider' END AS ProvType
...
# COMMAND ----------
 ...
 ---gets people subject to community treatment order, community treatment order recall or conditional discharge only in March of the financial year by provider
 ...
 CASE WHEN MHS10.OrgIDProv LIKE 'R%' OR MHS10.OrgIDProv LIKE 'T%' THEN 'NHS Trust' ELSE 'Independent Health Provider' END AS ProvType
   ...
   LEFT JOIN $db_output.mha_mhs09prov_prep AS MHS09 
      ...
      INNER JOIN $db_source.MHS001MPI AS PRSN 
       ...
       WHERE MHS11.Person_ID IS NULL ---no guardianship records
 			AND MHS09.Person_ID IS NULL ---no detention records
...
# COMMAND ----------
 ...
 ---combines all mental health data for each referral, mha record and hospital spell in March of the finacial year
...
# COMMAND ----------
 ...
 ---aggregates $db_output.mha_mhs08_additional_prep into different parts of the mental health act using LegalStatusCode
 SELECT		'Part 2' as MHA_Part, 
             'ENGLAND' as OrgType, 
             count(distinct Person_ID) as mhs09			
 FROM		$db_output.mha_mhs08_additional_prep				
 WHERE		LegalStatusCode in ('02', '03') ---Section 2 and 3 formal detentions
 UNION
 SELECT		'Part 2' as MHA_Part, 
 ...
 SELECT		'Part 3' as MHA_Part, 
             ...
             count(distinct Person_ID) as mhs09
 FROM		$db_output.mha_mhs08_additional_prep
 WHERE		LegalStatusCode in ('07','08','09','10','37','38','15','16','17','18','12', '13', '14') ---Part 3 detentions only
 UNION
 SELECT		'Part 3' as MHA_Part, 
             case when (length(OrgIDProv) = 3 and left(OrgIDProv, 1) in ("R", "T")) then "NHS Trust"
             else "Independent Health Provider" end as OrgType,
 ...
 SELECT		'Other' as MHA_Part, 
             ...
             count(distinct Person_ID) as mhs09
 FROM		$db_output.mha_mhs08_additional_prep
 WHERE		LegalStatusCode not in ('02','03','07','08','09','10','37','38','15','16','17','18','12', '13', '14') ---all other detentions
 UNION
 SELECT      'Other' as MHA_Part,
             case when (length(OrgIDProv) = 3 and left(OrgIDProv, 1) in ("R", "T")) then "NHS Trust"
             else "Independent Health Provider" end as OrgType, 
